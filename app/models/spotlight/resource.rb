module Spotlight
  ...
  class Resource < ActiveRecord::Base
    Rails.logger.warn("In Resource 5 AT:"+Time.now.strftime("%m%d %H:%M:%S:%L")+"")
    ...
    def save_and_index(*args)
	  Rails.logger.warn("In Resource.rb save_and_index AT:"+Time.now.strftime("%m%d %H:%M:%S:%L")+"")
	  Rails.logger.warn("save_and_index with start" + args.inspect )
      save(*args) && reindex_later
	  Rails.logger.warn("save_and_index end" + args.inspect )
    end
	def save
	...
	def self.generate_pdfs(allr = nil)
		...
		allr.each do |r|
			...
			value = `/bin/bash -l -c '/usr/bin/convert #{pages} #{pdfName} >> /home/exhibit/ww1-project/ww1-project/log/cron_log.log 2>&1'`
